...
export default function AdvancedFilter({ availableTags = [], searchParams, updateUrl }) {
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef(null);

  const currentTags = searchParams.getAll("tags[]");
  const currentMatchType = searchParams.get("tagMatchType") || "all";
        ...
        <div className="absolute z-50 mt-2 w-80 bg-white rounded-xl 
          ...
          <div className="p-5 space-y-4">
            <div className="flex items-center justify-between border-b 
              ...
              <h3 className="text-lg font-semibold text-green-800">Tag Filters</h3>
              {currentTags.length > 0 && (
                <button
              ...
              <div className="flex space-x-6">
                <label className="flex items-center space-x-2 text-sm 
                  ...
                  <span className="text-gray-700 group-hover:text-green-800 
                                 transition-colors duration-200">All tags</span>
                </label>
                <label className="flex items-center space-x-2 text-sm 
                                cursor-pointer group">
                  <input
                  ...
                  />
                  <span className="text-gray-700 group-hover:text-green-800 
                                 transition-colors duration-200">Any tag</span>
                </label>
              </div>
            </div>

            <div className="space-y-3">
              <p className="text-sm font-semibold text-[#0c3a2da5]">Available Tags</p>
