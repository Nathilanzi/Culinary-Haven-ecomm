...
export default function Favorites() {
  ...
  const [recipe, setRecipe] = useState(null); // Initially set recipe to null
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        setError("Failed to initialize user ID: " + err.message);
      }
    };
    initialize();
  }, []);
  ...
  const initializeUserId = async () => {
    ...
    if (!userId) {
      try {
        ...
        throw new Error("Error initializing user ID: " + error.message);
      }
    }
  ...
  const fetchFavoritesCount = async (userId) => {
    try {
      ...
      setError("Error fetching favorites count: " + error.message);
    }
  };
  ...
  const handleFavoriteToggle = async (recipeId, isFavorite) => {
    ...
    try {
      const response = await fetch("/api/favorites", {
        method: isFavorite ? "DELETE" : "POST", // DELETE to remove, POST to add
        headers: {
          "user-id": userId,
      ...
      if (response.ok) {
        await fetchFavorites(userId); // Refresh favorites list
        await fetchFavoritesCount(userId); // Refresh favorites count
      } else {
  ...
  const isFavorited = (recipeId) => favorites.some(fav => fav.recipeId === recipeId);
