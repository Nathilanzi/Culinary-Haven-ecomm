...
const CACHE_KEY_RECIPES_LIST = (params) => {
  const { page, limit, search, tags, ingredients, matchType } = params;
  // Sort arrays to ensure consistent cache keys regardless of order
  const tagsKey = tags ? [...tags].sort().join(',') : '';
  const ingredientsKey = ingredients ? [...ingredients].sort().join(',') : '';
  return `recipes_${page}_${limit}_${search || ""}_tags(${tagsKey})_ingredients(${ingredientsKey})_match(${matchType || "all"})`;
};
 ...
 * @param {string[]} [ingredients] - Array of ingredients to filter by
 * @param {string} [matchType] - How to match tags and ingredients ('all' or 'any')
...
export async function getRecipes(
  ...
  try {
    ...
    // Add each tag and ingredient as separate parameters
